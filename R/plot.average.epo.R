plot.average.epo <- function(epo.ev1,epo.ev2, res, t1=-500, t2=1000, expname)
{
  ...
  df <- data.frame(mode = c(rep(res$type1,nrow(meanEpo1)), rep(res$type2,nrow(meanEpo2))),
                   t=c(t,t),
                   PO3 = c(meanEpo1[,res$channels[1]], meanEpo2[,res$channels[1]]),
                   PO4 = c(meanEpo1[,res$channels[2]], meanEpo2[,res$channels[2]]),
  ...
  plPO3 <- plot_ch(df, "PO3")  
  plPO4 <- plot_ch(df, "PO4")  
  ...
  grid.arrange(plPO3, plPO4, plPO7, plPO8,
               plHeOGl, plVdn,               
  ...
  pl <- arrangeGrob(plPO3, plPO4, plPO7, plPO8,
                    plHeOGl, plVdn,               
